...
public class OutdoorActivity extends AppCompatActivity implements Button.OnClickListener{

    private TubeSpeedometer gaugeTemp, gaugeHumidity, gaugePressure, gaugeCO2, gaugePM;
    int BT_Img[] = {R.id.IMG_BACK, R.id.IMG_SET, R.id.IMG_LOVE};

    TextView TXVShow, TXV_GPS, TXVUpdateTime;
    LinearLayout myLinearlayout, LinearCO2, LinearPM;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TXV_GPS = (TextView) findViewById(R.id.TXV_GPS);
        ...
        getSupportActionBar().setTitle("Outdoor");
    ...
    private void setupGaugePressure(){
        gaugePressure = (TubeSpeedometer) findViewById(R.id.GAUGE_PRESSURE);
        gaugePressure.setSpeedTextPosition(Gauge.Position.CENTER);
        gaugePressure.setUnit("pa");
        gaugePressure.setSpeedTextSize(GaugeTextSize);
        gaugePressure.setUnitTextSize(GaugeUnitSize);
        gaugePressure.setTextSize(GaugeBarTextSize);
        gaugePressure.speedTo(0);
    }
//    private void setupGaugeCO(){
//        gaugeCO = (TubeSpeedometer) findViewById(R.id.GAUGE_PRESSURE);
                                ...
                                try {
                                    ...
                                    if(each_data[i].replaceAll("\\s", "").equals("TS")){
                                        ...
                                        for(int k=0;k<data_split.length;k++){
                                            if(data_split[0].substring(0,6).equals("DNLIST")){
//                                                Log.e("fff", "s");
                                                String DS = data_split[0].substring(8, data_split[0].length());
                                                String NS = data_split[1].substring(0, data_split[1].length()-1);
                                                loc.add(DS+","+NS);
                                                if(VERBOSE) Log.e("DNS", DS+","+NS);
                                            }
//                                            Log.e("f"+String.valueOf(k), data_split[k]);
                                        }
                                    }
                    ...
                    if(!pressure.isEmpty()){
                        if(show_data_type == CHOOSE_OUTDOOR){
                            for(int i=0;i<pressure.size();i++){
                                ...
                                gaugePressure.speedTo(Float.parseFloat(pressure.get(i)));
                            }
                        }
                        else if(show_data_type == CHOOSE_INDOOR){
                            for(int i=0;i<pressure.size();i++){
...
//                                Loc_Data.put(t.get(i), loc.get(i));
                    ...
                    if(CO2Warning>=3||PMWarning>=3){
                        setDangerInterface();
                    }
                    else if(CO2Warning>=2||PMWarning>=2){
                        setBadInterface();
                    }
                    else if(CO2Warning>=1||PMWarning>=1){
                        setSoSoInterface();
                    }
                    else{
                        setGoodInterface();
    ...
    private void queryOutdoorData(String dateToQuery){
        myQuery.mode = 1;
        ...
        myQuery.Query(ref, time_name, "<=", dateToQuery, 1, "DESCENDING");
        ref = db.collection(UsingOutdoor+GPS_name);
        myQuery.Query(ref, time_name, "<=", dateToQuery, 1, "DESCENDING");
    }
